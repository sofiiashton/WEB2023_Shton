...
const displayTable = (teachersData) => {
  ...
  if (currentPage === 1) {
    previousPageLink.classList.add("table-pagination");
  ...
  for (let page = 1; page <= totalPages; page++) {
    ...
    if (page === currentPage) {
      link.classList.add("table-pagination");
  ...
  if (currentPage === totalPages + 1) {
    nextPageLink.classList.add("table-pagination");
    ...
    nextPageLink.addEventListener("click", (event) => {
      ...
      currentPage++;
      displayTable(teachers);
      if (currentPage > teachers.length / rowsPerPage) {
...
const fetchMoreTeachers = async () => {
  try {
    ...
    const data = await response.json();
    const newTeachers = formatTeachers(data);

    console.log(newTeachers);

    teachers = mergeLists(teachers, newTeachers);
    displayTeachers(teachers);
    displayTable(teachers);
    displayFavorites(teachers);
  } catch (error) {
