...
const formatTeachers = (data) => {
  const transformedData = data.results.map((teacher) => {
    return {
...
const fetchTeachers = async () => {
  try {
    ...
    teachers = formatTeachers(data);
    // const mergedTeachers = mergeLists(teachers, additionalUsers);
    displayTeachers(teachers);
    displayFavorites(teachers);
    countTeachersBySubject(teachers);
    displayPieChart(countTeachersBySubject(teachers));
  } catch (error) {
...
// lodash
// const validation = (teacherInfo) => {
//   const { full_name } = teacherInfo;
//   if (!_.isString(full_name) || !_.startsWith(full_name, _.upperCase(full_name))) {
//     throw new Error("Incorrect input of full name. Please try again.");
//   }
// };
...
function submitHandler(event) {
  ...
  const birthYear = parseInt(form.birthday.value);
...
// lodash
// const filterTeachers = (teachers, country, ageRange, gender, photoOnly, favorite) => {
//   return _.filter(teachers, teacher => {
//     const hasPhoto = teacher.picture_Large !== undefined && teacher.picture_thumbnail !== undefined;
//     return (
//       (country === undefined || teacher.country === country) &&
//       (ageRange === undefined || checkAgeRange(teacher.age, ageRange)) &&
//       (gender === undefined || teacher.gender === gender) &&
...
// const sortTeachers = (teachers, ascending = true, sortBy = "full_name") => {
//   if (sortBy === "age") {
//     return _.sortBy(teachers, 'age', ascending ? 'asc' : 'desc');
//   } else if (sortBy === "full_name") {
//     return _.sortBy(teachers, 'full_name', ascending ? 'asc' : 'desc');
//   } else if (sortBy === "b_day") {
//     return _.sortBy(teachers, teacher => new Date(teacher.b_date), ascending ? 'asc' : 'desc');
//   } else if (sortBy === "country") {
//     return _.sortBy(teachers, 'country', ascending ? 'asc' : 'desc');
//   }
//   return teachers;
// };
...
//   return _.filter(teachers, teacher => {
//     return (
//       _.includes(teacher.full_name.toLowerCase(), lowercaseSearchParam) ||
//       _.includes(teacher.note.toLowerCase(), lowercaseSearchParam) ||
//       _.includes(teacher.age.toString().toLowerCase(), lowercaseSearchParam)
//     );
//   });
// };
...
const openInfoPopup = (teacher) => {
  ...
  teacherNote.classList.add("teacher-note");
  teacherNote.textContent = teacher.note;

  // day.js
  const daysToBday = calculateDaysToBDay(teacher.b_date);
  const daysRemainingElement = document.createElement('p');
  daysRemainingElement.classList.add("teacher-note")
  daysRemainingElement.textContent = `Days remaining until next birthday: ${daysToBday}`;

  const addToFavoritesButton = document.createElement("button");
  addToFavoritesButton.textContent = "Add to Favorites";
  addToFavoritesButton.classList.add("button");
  addToFavoritesButton.classList.add("favorites");
  ...
  popupMain.appendChild(daysRemainingElement);
  popupMain.appendChild(addToFavoritesButton);
  popupMain.appendChild(mapContainer);
  ...
  L.marker([teacher.coordinates.Latitude, teacher.coordinates.Longitude]).addTo(
    map
  );
};

const calculateDaysToBDay = (birthday) => {
  const today = dayjs();
  const nextBirthday = dayjs(birthday).year(today.year());

  if (nextBirthday.isBefore(today)) {
    nextBirthday.add(1, 'year');
    // console.log(nextBirthday.add(1, 'year'));
    const daysUntilNextBirthday = nextBirthday.add(1, 'year').diff(today, 'day');
...
const countTeachersBySubject = (teachers) => {
  const subjectCount = new Array(courseList.length).fill(0);
  teachers.forEach((teacher) => {
    const subjectIndex = courseList.indexOf(teacher.course);
    if (subjectIndex !== -1) {
...
const displayPieChart = (subjectCount) => {
  const colors = [
    "#FF6384",
    "#36A2EB",
    "#FFCE56",
